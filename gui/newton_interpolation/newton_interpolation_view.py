...
class NewtonInterpolationView(ctk.CTkFrame):
    """Vista del método de Interpolación de Newton (diferencias divididas)."""
    ...
    def create_layout(self):
        self.sidebar = Sidebar(self, active_method="newton_interpolation")
        ...
        ctk.CTkLabel(
            header_frame, text="Interpolación de Newton (Diferencias Divididas)",
            font=self.title_font, text_color=COLOR_LIGHT_CYAN
        ).grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(
            header_frame,
            text="Construye el polinomio mediante la tabla de diferencias divididas y evaluación progresiva.",
    ...
    def _setup_left_panel(self, parent):
        ...
        ctk.CTkLabel(config_frame, text="Grado (opc):", font=self.label_font, text_color=COLOR_MUTED).grid(
            row=3, column=0, sticky="w", padx=(0, 8), pady=(0, 6))
        self.entry_degree = ctk.CTkEntry(config_frame, font=self.label_font, fg_color=COLOR_BG,
                                         border_color=COLOR_INTERACTIVE_BORDER, text_color=COLOR_TEXT, width=80)
        self.entry_degree.grid(row=3, column=1, sticky="w", pady=(0, 6))
        ...
        self.entry_func = ctk.CTkEntry(config_frame, font=self.label_font, fg_color=COLOR_BG,
                                       border_color=COLOR_INTERACTIVE_BORDER, text_color=COLOR_TEXT)
        self.entry_func.grid(row=4, column=1, sticky="ew", pady=(0, 6))
    ...
    def _setup_steps_tab(self):
        ...
        ctk.CTkLabel(
            self.tab_steps, text="Tabla de Diferencias Divididas y Evaluación",
            font=self.section_font, text_color=COLOR_LIGHT_CYAN
        ).grid(row=0, column=0, sticky="w", padx=10, pady=10)
        ...
        self.txt_steps.insert(
            "1.0",
            "Paso a paso:\n"
            "1. Se construye la tabla de diferencias divididas\n"
            "2. Se extraen los coeficientes de la diagonal\n"
            "3. Se evalúa progresivamente en el punto x deseado"
        )
    ...
    def clear_inputs(self):
        for e in [self.entry_x, self.entry_y, self.entry_eval, self.entry_func, self.entry_degree]:
            e.delete(0, "end")
    ...
    def solve_interpolation(self):
        ...
        degree_str = self.entry_degree.get().strip()
        degree = int(degree_str) if degree_str else None

        func_str = self.entry_func.get().strip()
        solver = NewtonInterpolation(x, y, eval_point, degree=degree)
        result = solver.solve()
        ...
        if not result["success"]:
            ...
            out += f"Coeficientes: {[f'{c:.6g}' for c in sol['coefficients']]}\n\n"
            if func_str:
                try:
                    import sympy
            ...
            for step in result["steps"]:
                ...
                elif step["type"] == "table_col":
                    steps_text += f"\n-- {step['description']} --\n"
                    table = step["table_snapshot"]
                    n_rows = len(table)
                    width = 10
                    for i in range(n_rows):
                        row_str = f"  x[{i}]={x[i]:.4g} |"
                        for j in range(n_rows):
                            val = table[i][j]
                            row_str += f" {val:>{width}.4g}" if val != 0 else " " * (width + 1) + " " + "0" + " " * (width - 2)
                        steps_text += row_str + "\n"
                elif step["type"] == "coefficients":
                    steps_text += f"\n{step['description']}\n\n"
                elif step["type"] == "evaluation":
                    steps_text += f"\n{step['description']}\n"
                    for row in step.get("rows", []):
                        steps_text += f"  {row['description']}\n"
    ...
    def _plot(self, x_data, y_data, eval_point, solution, func_str):
        ...
        ax.set_title("Interpolación de Newton (Diferencias Divididas)")
